...
{
	spi_master_write_command(dev, 0x81); // 设置对比度命令
	spi_master_write_command(dev, contrast); // 对比度值（0-255）
}

// 开启屏幕
void ssd1306_on(SSD1306_t * dev)
{
	spi_master_write_command(dev, 0xAF); // 开启显示命令
}

// 关闭屏幕
void ssd1306_off(SSD1306_t * dev)
{
	spi_master_write_command(dev, 0xAE); // 关闭显示命令
}
